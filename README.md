...
> 若商店链接尚未上线，可关注本仓库 Release 页面的说明；**Release 附件仅供开发者/审核参考，不建议普通用户解压安装。**
...
某上游 **限流或故障** 时自动("Gemini 429 → 切 Groq → …") 自动 failover，用户无需配置模型列表。

**隐私**：BYOK 的 Key **仅保存在本机浏览器**（`chrome.storage`），不上传至作者服务器；智能云路由 **不需要用户 Key**。详见扩展内 AI 设置区的说明文字。

**免责声明**：AI 输出仅供参考，签证与资格问题请以 **Home Affairs 官网** 为准。
...
本扩展 **免费使用**，无应用内广告与追踪。若对你规划 WHV 有帮助，欢迎：

- 📖 阅读更多攻略：[Jessie 的 WHV 专题页](https://www.jessieontheroad.com/zh/whv/)  
...
> 维护者：请将你的打赏二维码图片保存为 **`images/donate-qr.png`**（建议 400×400 以上 PNG），提交后即可在 README 与 GitHub 页面显示。
